	...
	<script type="text/javascript">
		...
		function reset() {
			...
			$('#recipe-keyword').val('<?= $recipe->keyword; ?>');
			$('#category-list').val('<?= $recipe->category_id; ?>');

			$('#preview-image').data('image_id', 0);
			$('#recipe-image').val('');

			var img = '<img class="preview" src="<?= base_url(); ?>images/website/<?= $recipe->image_name; ?>" style="width: 100%;">';
			$('#preview-image').append(img);
		}
		...
		function submit() {
			...
			if (name == '' || keyword == '' || description == '' || ingredients == '' || preparations == '' || categoryId <= 0) {
				submitQuery = false;
			...
			$.ajax({
				...
				type : 'POST',
				url : '<?= base_url() ?>admin/recipe/ajax_edit/<?= $recipe->id; ?>/'
			});
		}
	</script>
				...
				<div id="recipe-details" class="tab-pane fade in active">
					...
					<div class="container-fluid">
						...
						<div class="row">
							...
							<div class="col-lg-8 text-left">
								<textarea id="recipe-description" class="input-all" type="text" placeholder="Description.."><?= $recipe->description; ?></textarea>
							</div>
						</div>
						<hr class="separator"></hr>
						<div class="row">
							...
							<div class="col-lg-8 text-left">
								<textarea id="recipe-ingredients" class="input-all" type="text" placeholder="Ingredients.."><?= $recipe->ingredients; ?></textarea>
							</div>
						</div>
						<hr class="separator"></hr>
						<div class="row">
							...
							<div class="col-lg-8 text-left">
								<textarea id="recipe-preparations" class="input-all" type="text" placeholder="Preparation.."><?= $recipe->preparations; ?></textarea>
							</div>
						</div>
						<hr class="separator"></hr>
						<div class="row">
